...
subplot(1,1,1)
plot(agefine, accfine, 'k-', [1,18], [0,0], 'k:')
xlabel('\fontsize{16} Age')
ylabel('\fontsize{16} Acceleration (cm/sec^2)')
axis([1,18,-5,2.2])
...
for ilam=1:nlam
    ...
    hgtfdPar = fdPar(basis, Lfd, lambda);
    [hgtfd, df, gcv] = smooth_basis(hgtobs', age, hgtfdPar, wtvec);
    accest = eval_fd(age, hgtfd, 2);
    dfsave(ilam)    = df;
    gcvsave(ilam)   = gcv;
...
subplot(2,1,1)
plot(loglam, gcvsave, 'k-', [-1,-1], [400,600], 'b:')
ylabel('\fontsize{12} GCV')
subplot(2,1,2)
plot(loglam, sqrt(MSEsave(:,11)), 'k--', ...
     loglam, sqrt(MSEsave(:,19)), 'k-', ...
     loglam, sqrt(MSEsave(:,27)), 'k-.', ...
     [-1,-1], [0,1], 'k:')
...
subplot(3,1,1)
plot(age, sqrt(MSEsave(ilam,:)), 'k-')
ylabel('\fontsize{12} RMSE')
axis([1,18,0,2])

%  plot bias

subplot(3,1,2)
plot(age, Biasave(ilam,:), 'k-', [1,18], [0,0], 'k:')
ylabel('\fontsize{12} Bias')

%  plot standard error of estimate

subplot(3,1,3)
plot(age, sqrt(Varsave(ilam,:)), 'k-')
xlabel('\fontsize{12} Age')
ylabel('\fontsize{12} Std. Err.')
axis([1,18,0,1.1])
