...
if (nforce > 0) {
    ...
    for (iu in 1:nforce) {
      if (!inherits(ufdlist[[iu]], "fd")) {
        print(paste("UFDLIST[[",iu,"]] is not a functional data object.",sep=""))
        errorwrd <- TRUE
      } else {
	  ufdi   <- ufdlist[[iu]]
	  ...
	  if (any(urange != xrange)) {
          print(paste(
             "XRANGE and URANGE are not identical for UFDLIST[[",iu,"]].",sep=""))
             errorwrd <- TRUE
        }
      }
      afdPari <- awtlist[[iu]]
    	...
    	if (!inherits(afdi, "fd")) {
        print(paste(
	       "AFDI is not a functional data object for AWTLIST[[",iu,"]].",sep=""))
        errorwrd <- TRUE
      } else {
    	  basisi <- afdi$basis
    	  if (any(basisi$rangeval != urange)) {
          print(paste("Ranges are incompatible for AWTLIST[[",iu,"]].",sep=""))
          errorwrd <- TRUE
        }
  ...
  for (ivar in 1:nvar) {
    for (iu in 1:length(ufdlist[[ivar]])) {
        ...
        if (!inherits(ufdiviu, "fd")) {
          print(paste(
              "UFDLIST[[",ivar,",",iu,"]] is not a functional data object.",
              sep=""))
          errorwrd <- TRUE
        }
